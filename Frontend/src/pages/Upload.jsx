    ...
    onDrop: (acceptedFiles) => {
      ...
      setUploadedFiles((prevFiles) => [
        ...
        ...newFiles.map((file) => ({
          ...
          sizeDisplay: file.size < 102400 ? `${(file.size / 1024).toFixed(2)} KB` : `${(file.size / (1024 * 1024)).toFixed(2)} MB`,
        })),
      ]);
        ...
        <div className="flex flex-col my-4 gap-4 items-center">
          {uploadedFiles.length > 0 ? (
            uploadedFiles.map((file, index) => (
              ...
              >
                <div className="text-lg font-semibold break-words">{file.name}</div>

                <div className="text-sm text-[#DADADA]">{file.sizeDisplay}</div>
              </div>
            ))
          ) : (
